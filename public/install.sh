...
main() {
    INSTALL_DIR="$HOME/twineline"
    MANIFEST_URL="https://twineline.ai/releases/manifest.json"
    TMPDIR_INSTALL=""
    ...
    if [ -z "$PYTHON" ]; then
        ...
        if [ "$PLATFORM" = "macos" ]; then
            # Download the .pkg installer from python.org (no Homebrew needed)
    ...
    # Parse manifest safely - pass file path as argument, not interpolated
...
" "$MANIFEST_JSON" "$TAG")
    ...
    echo "Downloading TwineLine $TAG..."
    BUNDLE_PATH="$TMPDIR_INSTALL/twineline.tar.gz"
    ...
    if [ -n "$EXPECTED_SHA256" ]; then
        if command -v sha256sum >/dev/null 2>&1; then
            ...
            ACTUAL_SHA256="$EXPECTED_SHA256"  # Skip check
        fi
    ...
    echo ""
    echo "TwineLine $TAG installed successfully!"
    echo ""
    echo "To start TwineLine:"
    echo "  cd $INSTALL_DIR && source .venv/bin/activate && python run.py"
    echo ""
