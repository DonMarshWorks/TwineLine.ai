...
function Install-TwineLine {
    $ErrorActionPreference = "Stop"

    $InstallDir = "$env:USERPROFILE\twineline"
    $ManifestUrl = "https://twineline.ai/releases/manifest.json"
    $TempDir = Join-Path $env:TEMP "twineline-install-$(Get-Random)"

    try {
        New-Item -ItemType Directory -Force -Path $TempDir | Out-Null

        Write-Host "Installing TwineLine..." -ForegroundColor Cyan
        Write-Host ""
        ...
        Write-Host "Downloading TwineLine $tag..."
        $bundlePath = Join-Path $TempDir "twineline.tar.gz"
        ...
        Write-Host ""
        Write-Host "TwineLine $tag installed successfully!" -ForegroundColor Green
        Write-Host ""
        Write-Host "To start TwineLine:"
        Write-Host "  cd $InstallDir; .venv\Scripts\Activate.ps1; python run.py"
        Write-Host ""
